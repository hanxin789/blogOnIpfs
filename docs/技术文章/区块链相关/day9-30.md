...
//类似JavaScript的函数写法
 function changeNumber(uint128 Number) public {
  testdata = ++Number;
 }
...
/*在EVM中针对不能确定内存位置的复杂类型参数定义有多种位置标识符来显式定义变量存储位置 (map 数组 以及结构体)
位置标识符：store(持久的比如合约中的变量) memory(临时的仅在调用函数时使用,可修改,多用于形参与返回值) 
calldata(也是临时变量的位置标识符,但是只读) stack code logs,
